  ...
  .define((options, notifier, push) => {
    ...
    const WdaClient = {
      ...
      isSessionAlive: function() {
        ...
        log.info("verifying wda session using sessionId: " + this.sessionId)
        return new Promise((resolve, reject) => {
          request.get(`${this.baseUrl}/session/${this.sessionId}`).then(response => {
            let currentSessionId = JSON.parse(response).sessionId
            log.info("response detected during sessionId verification: " + currentSessionId)
            if (this.sessionId == currentSessionId) {
              log.info("current sessionId is good: " + this.sessionId)
              this.isAlive = true
            } else {
              log.error("current sessionId is NOT good: " + this.sessionId)
            }
          }).catch(err => {
            log.error("current sessionId is NOT good: " + this.sessionId)
            log.error(err)
...
// skelethon of the consiquent requests
/*
      startSession: function() {
        this.isSessionAlive().then(response => {
