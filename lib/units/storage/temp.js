            ...
            if (!err) {
              Object.keys(files).map((value) => {
                ...
                decompress(file.path, tmpDirPath)
                  .then(() => {
                    log.info('clearing tmp dir: ' + tmpDirPath)
                  ...
                  .catch(() => {
                      ...
                      log.info('ios agrs:', args)
                      let iosGo = spawn('ios', args, {
                        cwd: tmpDirPath,
                        shell: true
                      })
                      ...
                      iosGo.stdout.on('data', data => {
                        // @TODO add handler
                        log.info('iosDeploy.stdout data :', data.toString())
                        // var output = data.toString()
                      })

                      iosGo.stderr.on('data', data => {
                      ...
                      iosGo.on('close', code => {
                        ...
                        if (code) {
                          log.important(
                            `iosGo closed with code: ${code.toString()}`
                          )
                        }
                      })
                    }
